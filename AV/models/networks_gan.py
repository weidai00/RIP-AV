...
if __name__ == '__main__':
    model = NLayerDiscriminator(input_nc=3, ndf=64,num_classes_D=1, n_layers=3, norm_layer=nn.BatchNorm2d)
    x = torch.randn(1, 3, 256, 256)
    o = model(x)
    label_shape = [1,1, o.shape[2]]
    # 0, 1
    label_real = torch.zeros(label_shape)
    label_fake = torch.ones(label_shape)
    print(label_real.shape)

    k = nn.BCEWithLogitsLoss()(o, label_real)
    print(k)
