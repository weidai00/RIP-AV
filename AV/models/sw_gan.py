...
from models.network import PGNet, set_requires_grad

from models import networks_gan
from loss import multiclassLoss, multidiceLoss, multiLabelLoss, multiclassLossAV, multiHausdorffDTLoss, centernessLoss
import os, copy
...
class SW_GAN():
    ...
    def __init__(self, opt, isTrain=True):
        ...
        #set_requires_grad(self.netG.sn_unet,requires_grad=False)
        if self.use_cuda:
    ...
    def set_input(self, step, train_data=None, train_label_data=None, train_label_data_centerness=None,
                  train_data_mask=None,train_data_mask2= None,train_patch_data=None,
        ...
        patch_data, patch_label, label_centerness, data, label = get_patch_trad_5(opt.batch_size,
                                                                                  ...
                                                                                  patch_size1=self.cfg.patch_size_list[0],
                                                                                  patch_size2=self.cfg.patch_size_list[1])
    ...
    def forward_ema(self):
        ...
        if self.use_global_semantic:
            if self.step <= self.cfg.global_warmup_step:
               ...
               self.update_momentum_ema(self.netG.sn_unet, self.netG.base_layers_global_momentum, 0)
    ...
    def log(self, logger):
        logger.draw_prediction(self.pre_target, self.patch_label_input, self.centerness_maps, self.label_centerness_map,
                               self.step)
    ...
    def test(self, result_folder,dataset=''):
        print("-----------start to test-----------")
        modelEvalution(self.step, self.netG.state_dict(),
                       result_folder,
                       use_cuda=self.cfg.use_cuda,
                       dataset=self.cfg.dataset if dataset=='' else dataset,
    ...
    def backward_D(self, isBackward=True):
        ...
        self.loss_D = self.loss_D * opt.lambda_GAN_D
    ...
    def backward_G(self, isBackward=True):
        ...
        #self.loss_G_Hu = 0.05 * self.criterionHu(self.pre_target, self.patch_label_input)
