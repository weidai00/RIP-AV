...
def AVclassifiationMetrics_post_process(PredAll1, PredAll2, VesselPredAll, LabelAll1, LabelAll2, LabelVesselAll,
...
def AVclassifiationMetrics_post_process_v2(PredAll1, PredAll2, VesselPredAll, LabelAll1, LabelAll2, LabelVesselAll,
                                          DataSet=0, onlyMeasureSkeleton=False, strict_mode=False):
    """
    predAll1: 动脉预测结果
    predAll2: 静脉预测结果
...
def AVclassifiation(out_path,PredAll1, PredAll2, VesselPredAll, DataSet=0):
    """
    predAll1: 动脉预测结果
    predAll2: 静脉预测结果
    VesselPredAll: 血管预测结果
    ...
    for ImgNumber in range(ImgN):
        ...
        VesselSeg = VesselProb >= 0.5
        VesselSeg = binaryPostProcessing3(VesselSeg, removeArea=50, fillArea=20)
        ...
        ArteryVeinPredImg_v2[ArteryPred2 > 0] = (255, 0, 0)
        ArteryVeinPredImg_v2[VeinPred2 > 0] = (0, 0, 255)
        Image.fromarray(ArteryVeinPredImg_v2).save(os.path.join(out_path,f'av_{ImgNumber}_with_dis_V2.png'))



if __name__ == '__main__':
    pro_path_post = r'G:\Tencent\WeChat\WeChat Files\wxid_3yy1snyq84jn21\FileStorage\File\2023-05\ProMap_testset_after_process_v4.npy'
    ...
    AVclassifiationMetrics_post_process_v2(ps[:, 0:1, :, :] , ps[:, 1:2, :, :],
                                          ps[:, 2:, :, :] , ls[:, 0:1, :, :], ls[:, 1:2, :, :],
                                          ls[:, 2:, :, :], DataSet=ps.shape[0])
