...
"""
import os
from opt_UKBB import AV_based_vessel,CAM
...
from AV_out_eval_metrics import AVclassifiation
def test(cfg):
...
def modelEvalution_out(i,net,savePath, use_cuda=False, dataset='DRIVE', is_kill_border=True, input_ch=3, strict_mode=True,config=None):
    ...
    dataset_dict = {'out':'out','DRIVE': 'AV_DRIVE', 'hrf': 'HRF_AVLabel_191219', 'ukbb': 'ukbb','all_combine':'all_combine'}
    ...
    n_classes = 3
    Net = PGNet(resnet=config.use_network, input_ch=input_ch, num_classes= n_classes, use_cuda=use_cuda, pretrained=False, centerness=config.use_centerness, centerness_map_size=config.centerness_map_size,use_global_semantic=True)
    ...
    AVclassifiation(data_path,ArteryPredAll,VeinPredAll,VesselPredAll,test_image_num,onlyMeasureSkeleton=False,strict_mode=strict_mode)
...
def GetResult_out(Net, k, use_cuda=False, dataset_name='DRIVE', is_kill_border=True, input_ch=3, config=None):
    ...
    #     Mask = cv2.resize(Mask, config.reszie_w_h, interpolation=cv2.INTER_NEAREST)
